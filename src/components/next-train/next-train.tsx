    ...
    <div className="center">
      <Container>
        ...
        {nextTrainData.length > 0 ? nextTrainData.map((trainData) => {
          return (
            <li className="train-list" key={trainData.ttnt}>{trainData.time} 
              {(Date.parse(trainData.time) - timeStamp.varTwo.getTime()) < 0 ? 
                <span><br></br>走左喇, 下架啦！</span>
                : 
                <span><br></br>Remaing Time: {msToTime(Date.parse(trainData.time) - timeStamp.varTwo.getTime())}</span>
              }
            </li>
          )
        }) : "已經無車啦"}
